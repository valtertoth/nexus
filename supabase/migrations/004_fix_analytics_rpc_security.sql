-- ============================================
-- Fix: Analytics RPCs accept arbitrary org_id (HIGH severity)
-- Add auth.uid() verification to ensure the calling user
-- belongs to the requested organization.
-- ============================================

-- Daily conversation stats (with org ownership check)
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'access_denied: user does not belong to this organization';
  END IF;
...
-- Agent performance metrics (with org ownership check)
...
-- AI usage summary (with org ownership check)
